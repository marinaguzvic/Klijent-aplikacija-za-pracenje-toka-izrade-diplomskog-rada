 ...
 */
public class OdobriDiplomskiRadAction extends AbstractAction{
    ...
    public boolean callToService(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            DiplomskiRadDTO d = ((DiplomskiRadClient)client).odobri(id);
            request.setAttribute("diplomskiRad", d);
            request.setAttribute(IConstants.SUCCESS_MESSAGE, "Sistem je uspešno odobrio temu diplomskog rada");
            return true;
        } catch (Exception ex) {
            ex.printStackTrace();
